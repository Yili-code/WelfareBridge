...
export default function ProfileWizard({ onComplete, onCancel }: { onComplete: (draft: Draft) => void; onCancel?: () => void }) {
 ...
 const next = () => {
   ...
   onComplete({ nickname: nickname.trim() || RELATIONS.find(r => r.value === relation)!.label, relation, region: noHousehold ? '' : region, currentRegion: sameCity ? region : currentRegion, age: exactAge === '' ? null : Number(exactAge), screening: answers, ...profileTags(answers) });
 };
 ...
 return <div className="flex h-full flex-col">
   ...
   <div className="mt-6 flex items-center justify-between border-t border-slate-200 pt-5"><button type="button" onClick={() => step === 0 ? onCancel?.() : setStep(step - 1)} disabled={step === 0 && !onCancel} className="rounded-lg px-4 py-2.5 text-sm text-ink-400 disabled:invisible">{step === 0 ? '取消' : '上一步'}</button><button type="button" onClick={next} disabled={!canNext} className="rounded-lg bg-brand-500 px-6 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400">{step === QUESTIONS.length - 1 ? '完成建檔' : '下一步'}</button></div>
 </div>;
}
