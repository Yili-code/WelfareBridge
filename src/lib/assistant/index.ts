...
async function turn(state: AssistantState, profile: Profile | null, userText?: string): Promise<AssistantTurn> {
 ...
 return { search: data.search || undefined, state: { step: state.step + 1, answers: {}, turnCount: data.turn_count, candidateCount: data.candidate_count, completed: data.completed, guidanceProfile: data.guidance_profile, questionAttribute: data.question_attribute, history: [...history, { role: "assistant", content: data.reply }] }, replies: [data.reply], quickReplies: data.quickReplies, multiSelect: false, allowFreeText: !data.completed, draft };
}
