...
| 分頁 | 功能 |
| --- | --- |
| 查詢補助與服務 | 關鍵字搜尋、地區／補助領域／補助類型／適用對象篩選，每項補助標示「可能符合／需要進一步確認／目前較不符合」；點卡片看詳情（資格比對原因、金額、申請方式、附件、官方連結） |
| 我的資料卡 | 十題資料卡（可為家人分別建立），比對結果與主動提醒；聊天小幫手問到的資料會列在這裡，可修改或刪除 |
...
`/api/appeals`、`/api/demands` 由 Next.js 處理（存在 `data/welfare.sqlite`），其餘 `/api/*` 代理到 FastAPI；前台只呼叫 `/api/public/*`。資料卡存在瀏覽器 localStorage，尚非跨裝置同步。
