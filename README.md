...
使用者介面只有一個首頁（設計依 welfare-ui4），不顯示任何作業或除錯資訊：

| 分頁 | 功能 |
| --- | --- |
| 查詢補助與服務 | 還沒有資料卡時上方顯示建立邀請；關鍵字搜尋、地區／補助領域／補助類型／適用對象篩選、依截止日排序，每項補助標示「可能符合／需要進一步確認／目前較不符合」與「還有 N 天截止」；點卡片看詳情（資格比對原因、金額、申請方式、附件、官方連結） |
| 我的資料卡 | 資料卡在小視窗裡一次一題填寫（可為家人分別建立）；結果頁用大卡片分成「可能符合」「需要進一步確認」，上方「補這幾題」可直接回答最常缺的資料並重新比對，可列印／存成 PDF；小幫手問到的資料可修改或刪除 |
| 收藏清單 | 收藏的補助依截止日排列，7 天內截止會在頁面上方提醒；已下架的另外列出；可列印成申請清單 |
| 訴求專區 | 公開提出找不到的服務、附議；達 10 則進入待認領，由機關認領與回報處理結果 |

右下角「福利小幫手」依比對結果挑最能確認資格的問題來問；回答會**寫進資料卡**，主畫面的比對結果隨即更新（聊天視窗只顯示寫進了什麼、結果怎麼變）。長者模式放大字級與按鈕。
舊網址 `/dashboard`、`/my-benefits` 會導回首頁。細節見 [使用者介面](docs/user-interface.md)。

承辦人員用的頁面不從首頁連結：

| 路徑 | 功能 |
| --- | --- |
| `/admin` | 需求登記、訴求專區認領與回報、資源上架、媒合診斷（需後台密碼） |
| `/data-center` | 搜尋補助、官方原文、爬取與處理狀態 |
| `/data-center/{id}` | 原始資料、資格規則與證據 |
| `/registry` | 屬性登錄表與類別 |

比對分層：**可能符合**＝戶籍、年齡、學制、身分等主要資格（資格骨幹）都已比對相符；**需要進一步確認**＝沒有明確不符但還缺資料，會列出要補的欄位；**目前較不符合**＝已確認有條件不符。初步符合不等於機關核准。設計與回測數字見 [資格骨幹與分層媒合](docs/eligibility-core.md)。

`/api/appeals`、`/api/demands` 由 Next.js 處理（存在 `data/welfare.sqlite`），其餘 `/api/*` 代理到 FastAPI；前台只呼叫 `/api/public/*`。資料卡與收藏存在瀏覽器 localStorage，尚非跨裝置同步。
